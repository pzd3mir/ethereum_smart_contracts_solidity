...
contract realEstate {
	...
	constructor (string memory _propertyID, string memory _propertySymbol, address _mainPropertyOwner, uint8 _tax) {
	    ...
	    rentalLimit = 12;
	    rentalLimitBlocks = rentalLimit * blocksPer30Day;
	}
	...
	function _transfer(address _to, uint256 _value) private returns (bool success) {
		require(shares[msg.sender] >= _value);
	...
	function limitadvancedrent(uint8 _monthstolimit) public{
   ...
   function distribute() public onlyGov {
        ...
        for (uint256 s = 0; s < stakeholders.length; s += 1){
            ...
            uint256 _shares = sharesOf(stakeholder);
...
/*
    ...
    */
    ...
    function payRent(uint8 _months) public payable isMultipleOf onlyTennant{
            ...
            if (rentpaidUntill[tennant] == 0) {
                rentpaidUntill[tennant] = block.number + _additionalblocks;
            }
            else {
                rentpaidUntill[tennant] = rentpaidUntill[tennant] +_additionalblocks;
            }
            }
